...
\frametitle{Statistical remarks}
    ...
    \begin{enumerate}
        ...
        \item Random data (e.g., observed data) is characterized using
...
\frametitle{Rejection region}
	...
	\begin{example1}
        ...
        the trasgenic mice equals 2~cm/sec if their sample mean speed is much
...
\frametitle{p-value}
    ...
    \begin{definition}[p-value]
        ...
        the probability that the test statistic is as extreme or mre than the observed
        test statistic value, when the null hypothesis is true (see figures
        below)..
...
\frametitle{Taxonomy of hypothesis tests for the the mean}
    ...
    \begin{itemize}
        \item The previous tests worked well because the sample statistic for
            the sample mean was approximately normal since

            \begin{enumerate}

                \item samples were Gaussian, with known variance, with any
                    sample size.

                \item the number of samples was sufficiently large (e.g.,
                    $n>30$) and samples were iid from any distribution
                    with mean $\mu$ and finite variance $\sigma^2$, known or
                    estimated by the sample variance $s^2$ (central limit
                    theorem).

            \end{enumerate}

        \item but what if samples are Gaussian, with unknown variance, and a
            small number $n$ of samples?
